...
export function resolveHost(project) {
...
export function generatePm2Config(project) {
...
export function generateSystemdService(project) {
    const slug = projectSlug(project);
    return `[Unit]
...
export function generateNginxConfig(project) {
...
export function generateApacheConfig(project) {
    const host = resolveHost(project);
    const slug = projectSlug(project);
    const useHttps = project.domain.useHttps;
...
export function generateCertbotScript(project) {
    const host = resolveHost(project);
...
export function generateFirewallScript(project) {
...
export function generateDeployScript(project) {
    const slug = projectSlug(project);
    const proxy = project.domain.reverseProxy;
    return `#!/usr/bin/env bash
...
export function generateDeploymentBundle(project) {
    const slug = projectSlug(project);
    const files = {
    ...
    if (project.domain.reverseProxy === 'nginx')
        files['nginx.conf'] = generateNginxConfig(project);
    if (project.domain.reverseProxy === 'apache')
        files['apache.conf'] = generateApacheConfig(project);
    if (project.domain.useHttps)
        files['certbot.sh'] = generateCertbotScript(project);
    return files;
}
